...
function Page({ params }: { params: { id: string } }) {
	...
	const [needPick, setNeedPick] = useState(socket.id == localInfos?.redTeam?.user ? false : socket.id == localInfos?.blueTeam?.user ? true: false);
	...
	const allIds = redTeamBans.concat(redTeamPicks, blueTeamBans, blueTeamPicks);
...
socket.on("changePlayer", () => {
	...
	const Champion = ({ champion }) => {
		return (
			<div className="relative champCard" style={{ filter: localInfos.disabled?.includes(champion.id) || selectedChamp === champion.id ? 'opacity(0.5)' : 'opacity(1)' }} onClick={() => !localInfos.disabled?.includes(champion.id) && needPick && chooseChamp(champion)}>
				<div className="imageStyling cursor-pointer">
					<Image
						width={100}
						height={100}
						alt="test"
						style={{ filter: "none", objectFit: "cover" }}
	...
	const ListeChampions = ({ champions }) => {
		return (
			<div className="flex flex-row flex-wrap gap-5 h-[93%] overflow-auto justify-between items-between">
	...
	if (!currentPhase) {
		...
		if (isRed) {
			socket.emit("changePlayer", params.id, actualValue.blueTeam.user);
		} else{
			socket.emit("changePlayer", params.id, actualValue.redTeam.user);
		}
	}

	if (isRed) {
		if (currentPhase && currentPick <= 1) {
			setCurrentPick(currentPick + 1)
			if (currentPick === 1) {
				setNeedPick(false)
				socket.emit("changePlayer", params.id, actualValue.blueTeam.user);
		...
		if (currentPick === 3 && currentPhase) {
			setCurrentPick(currentPick + 1)
			setNeedPick(false)
			socket.emit("changePlayer", params.id, actualValue.blueTeam.user);
		}

		if (currentPhase && currentPick === 2) {
			setCurrentPick(currentPick + 1)
			if (currentPick === 2) {
	...
	if (isBlue) {
		if (currentPhase && currentPick === 0) {
			setCurrentPick(currentPick + 1)
			setNeedPick(false)
			socket.emit("changePlayer", params.id, actualValue.redTeam.user);
		}

		console.log(currentPick, 'test')
		if (currentPick > 2  && currentPhase) {
			setCurrentPick(currentPick + 1)
			if (currentPick === 4) {
				setNeedPick(false)
				socket.emit("changePlayer", params.id, actualValue.redTeam.user); 
		...
		if (currentPhase && (currentPick === 1 || currentPick === 2)) {
			setCurrentPick(currentPick + 1)
			if (currentPick === 2) {
				setNeedPick(false)
				setCurrentPhase(false)
	...
	const redpicks = localInfos?.redTeam.pick.map((pick, index) => (
		...
		style={{ backgroundImage: `url(${pick.squarePortraitPath})` }}
		></div>
	));
	...
	const renderBlueTeamBans = () => {
		...
		const blueBans = localInfos?.blueTeam.ban.map((ban, index) => (
			<div
				...
				style={{ backgroundImage: `url(${ban.squarePortraitPath})` }}
			></div>
		));
	...
	const handleFinish = () => {
		if (needPick) {
			let newlocalInfos = localInfos

			if (needPick) {
				if (isRed) {
					console.log(localInfos.redTeam.ban[currentPick], newlocalInfos, currentPhase)
					if (currentPhase) {
						if (!localInfos.redTeam.pick[currentPick]) {
							newlocalInfos.redTeam.pick[currentPick] =     {
								id: -1,
								name: "None",
								alias: "None",
								squarePortraitPath: "/out/-1.png",
						...
						if (!localInfos.redTeam.ban[currentBan]) {
							newlocalInfos.redTeam.ban[currentBan] =     {
								id: -1,
								name: "None",
								alias: "None",
								squarePortraitPath: "/out/-1.png",
					...
					if (currentPhase) {
						if (!localInfos.blueTeam.pick[currentPick]) {
							newlocalInfos.blueTeam.pick[currentPick] =     {
								id: -1,
								name: "None",
								alias: "None",
								squarePortraitPath: "/out/-1.png",
						...
						if (!localInfos.blueTeam.ban[currentBan]) {
							newlocalInfos.blueTeam.ban[currentBan] =     {
								id: -1,
								name: "None",
								alias: "None",
								squarePortraitPath: "/out/-1.png",
			...
			console.log(newlocalInfos, 'finished')
	...
	return (
		<div style={{ height: "100vh" }}>
			<div className="h-[17%]">
				{isVisible && <Timer restartCallback={() => restartTimer()} onFinish={() => handleFinish()} />}
				</div>
		...
		<div onClick={() => needPick && validChoice()}>
			click
		</div>
		<div className="bg-blue flex items-center justify-center flex-row gap-5 ctn-ban">
					{renderRedTeamBans()}
